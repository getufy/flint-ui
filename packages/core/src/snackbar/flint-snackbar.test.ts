...
async function make(props: Partial<{
    ...
    anchorOrigin: FlintSnackbar['anchorOrigin'];
    pauseOnHover: boolean;
    closable: boolean;
    variant: FlintSnackbar['variant'];
}> = {}): Promise<FlintSnackbar> {
    const el = await fixture<FlintSnackbar>(html`
        <flint-snackbar
...
function snackbarDiv(el: FlintSnackbar) {
    return el.shadowRoot!.querySelector<HTMLElement>('.snackbar')!;
}

describe('flint-snackbar', () => {
    ...
    it('renders slotted content in default slot', async () => {
        const el = await fixture<FlintSnackbar>(html`
            <flint-snackbar>Custom slotted content</flint-snackbar>
        `);
        const slot = el.shadowRoot!.querySelector<HTMLSlotElement>('.message slot')!;
    ...
    it('renders slotted action content', async () => {
        const el = await fixture<FlintSnackbar>(html`
            <flint-snackbar open><button slot="action">UNDO</button></flint-snackbar>
        `);
    ...
    it('shows action wrapper when slot has content', async () => {
        const el = await fixture<FlintSnackbar>(html`
            <flint-snackbar open><button slot="action">X</button></flint-snackbar>
        `);
        await el.updateComplete;
        const actionDiv = el.shadowRoot!.querySelector<HTMLElement>('.action')!;
    ...
    it('fires flint-snackbar-open when opened', async () => {
        const el = await make();
        const spy = vi.fn();
        el.addEventListener('flint-snackbar-open', spy);
        el.open = true;
        await el.updateComplete;
    ...
    it('fires flint-snackbar-close when closed via close()', async () => {
        const el = await make({ open: true, autoHideDuration: 0 });
        const spy = vi.fn();
        el.addEventListener('flint-snackbar-close', spy);
        el.close();
        await el.updateComplete;
        expect(spy).toHaveBeenCalledOnce();
        const event = spy.mock.calls[0][0] as CustomEvent;
    ...
    it('fires flint-snackbar-close when open set to false directly', async () => {
        const el = await make({ open: true, autoHideDuration: 0 });
        const spy = vi.fn();
        el.addEventListener('flint-snackbar-close', spy);
        el.open = false;
    ...
    it('clears timer on disconnect', async () => {
        ...
        const el = await fixture<FlintSnackbar>(
            html`<flint-snackbar .autoHideDuration=${3000} open></flint-snackbar>`,
    ...
    it('accepts auto-hide-duration attribute', async () => {
        const el = await fixture<FlintSnackbar>(html`
            <flint-snackbar auto-hide-duration="2000"></flint-snackbar>
        `);
