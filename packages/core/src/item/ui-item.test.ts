...
describe('ui-item — corner cases', () => {
    ...
    it('does not override role if already set on separator', async () => {
        const el = await fixture<UiItemSeparator>(html`<ui-item-separator role="none"></ui-item-separator>`);
        expect(el.getAttribute('role')).toBe('none');
    });

    it('does not override aria-orientation if already set on separator', async () => {
        const el = await fixture<UiItemSeparator>(html`<ui-item-separator aria-orientation="vertical"></ui-item-separator>`);
