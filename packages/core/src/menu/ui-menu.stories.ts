...
**Components:**
- \`ui-menu\` — Root; manages open/close state, keyboard navigation, and positioning
- \`ui-menu-item\` — Interactive option (slots: \`icon\`, \`shortcut\`). Supports \`disabled\`
- \`ui-menu-group\` — Labeled group of items (set \`label\` for group heading)
- \`ui-menu-divider\` — Visual separator between items or groups

**Composition:** \`ui-menu\` > (\`ui-menu-item\` | \`ui-menu-group\` > \`ui-menu-item\`* | \`ui-menu-divider\`)*
