...
**Components:**
- \`ui-tabs\` — Root; manages selected tab state, keyboard navigation, and scroll buttons
- \`ui-tab-list\` — Container for tab triggers (renders the tab bar with active indicator)
- \`ui-tab\` — Individual tab trigger (slots: \`icon\`). Set \`value\` to identify it
- \`ui-tab-panel\` — Content panel; matched to a tab via \`value\`

**Composition:** \`ui-tabs\` > \`ui-tab-list\` > \`ui-tab\`* + \`ui-tab-panel\`*
