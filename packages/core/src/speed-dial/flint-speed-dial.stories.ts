...
const actions = () => html`
    ...
    <flint-speed-dial-action tooltip-title="Print">🖨️</flint-speed-dial-action>
    <flint-speed-dial-action tooltip-title="Share">🔗</flint-speed-dial-action>
...
const stageCorner = (content: unknown, corner = 'bottom-right') => {
    ...
    return html`
        <div style="
            position:relative;
            height:280px;
            background:var(--flint-muted-background, #f8fafc);
...
export const Playground: Story = {
    render: (args) => stageCorner(html`
        <flint-speed-dial
            .open=${args.open}
            .direction=${args.direction as FlintSpeedDial['direction']}
            ?hidden=${args.hidden}
            ?persistentTooltips=${args.persistentTooltips}
            ?isTouch=${args.isTouch}
            .closeIcon=${args.closeIcon}
            aria-label="Speed dial"
            @flint-speed-dial-open=${(e: Event) => { (e.target as FlintSpeedDial).open = true; }}
            @flint-speed-dial-close=${(e: Event) => { (e.target as FlintSpeedDial).open = false; }}
        >
            ${actions()}
        </flint-speed-dial>
    `, 'bottom-right'),
...
export const Controlled: Story = {
    render: () => {
        ...
        function getSD() {
            return document.getElementById('sd-controlled') as FlintSpeedDial | null;
        }
        ...
        return html`
            <flint-stack direction="column" gap="16px" style="font-family:Inter,sans-serif;">
                ...
                if (sd && !sd.open) { sd.open = true; syncLabel(true); }
                    ...
                    <button
                        style="padding:6px 14px;border-radius:6px;font-size:.8rem;font-family:inherit;
                               background:var(--flint-background, #fff);color:#374151;border:1px solid #e2e8f0;cursor:pointer;"
                        @click=${() => {
                const sd = getSD();
                ...
                <div style="
                    ...
                    background:var(--flint-muted-background, #f8fafc);border:1px solid #e2e8f0;border-radius:8px;overflow:hidden;
                ">
                    <div style="position:absolute;bottom:24px;right:24px;">
                        <flint-speed-dial
                            id="sd-controlled"
                            aria-label="Controlled speed dial"
                            @flint-speed-dial-open=${() => syncLabel(true)}
                            @flint-speed-dial-close=${(e: Event) => {
                (e.target as FlintSpeedDial).open = false;
                syncLabel(false);
            }}
                        >
                            ${actions()}
...
export const PersistentTooltips: Story = {
    render: () => html`
        <flint-stack direction="column" gap="16px" style="font-family:Inter,sans-serif;">
            <div style="
                padding:10px 14px;background:var(--flint-info-background, #eff6ff);border:1px solid #bfdbfe;
            ...
            ">
                <div style="position:absolute;bottom:24px;right:80px;">
                    ...
                    >
                        <flint-speed-dial-action tooltip-title="Copy link">🔗</flint-speed-dial-action>
                        <flint-speed-dial-action tooltip-title="Edit">✏️</flint-speed-dial-action>
                        <flint-speed-dial-action tooltip-title="Delete">🗑️</flint-speed-dial-action>
                    </flint-speed-dial>
                </div>
            </div>
        </div>
...
export const Hidden: Story = {
    render: () => html`
        <flint-stack direction="column" gap="16px" style="font-family:Inter,sans-serif;">
            <flint-stack direction="row" alignItems="center" gap="10px">
                <button
                    style="
...
export const DisabledActions: Story = {
    render: () => stageCorner(html`
        ...
        >
            ...
            <flint-speed-dial-action tooltip-title="Print (unavailable)" name="print" disabled>🖨️</flint-speed-dial-action>
            <flint-speed-dial-action tooltip-title="Share" name="share">🔗</flint-speed-dial-action>
            <flint-speed-dial-action tooltip-title="Delete (unavailable)" name="delete" disabled>🗑️</flint-speed-dial-action>
        </flint-speed-dial>
    `, 'center'),
};
        ...
        <flint-stack direction="column" gap="16px" style="font-family:Inter,sans-serif;">
            ...
            ">
                <div style="position:absolute;bottom:24px;right:80px;">
                    ...
                    >
                        <flint-speed-dial-action tooltip-title="Copy link" name="copy-link">🔗</flint-speed-dial-action>
                        <flint-speed-dial-action tooltip-title="Edit" name="edit">✏️</flint-speed-dial-action>
                        <flint-speed-dial-action tooltip-title="Delete" name="delete">🗑️</flint-speed-dial-action>
                    </flint-speed-dial>
                </div>
            </div>
        </div>
            ...
            ">
                <div style="position:absolute;bottom:24px;right:24px;">
                    <flint-speed-dial
                        ...
                        @flint-speed-dial-open=${(e: Event) => { (e.target as FlintSpeedDial).open = true; }}
                        @flint-speed-dial-close=${(e: Event) => { (e.target as FlintSpeedDial).open = false; }}
                        @flint-speed-dial-action-click=${(e: CustomEvent) => {
                            const label = document.getElementById('sd-last-action');
                            if (label) label.textContent = `${e.detail.name} (${e.detail.tooltipTitle})`;
                        }}
                    >
                        <flint-speed-dial-action name="copy"  tooltip-title="Copy">📋</flint-speed-dial-action>
                        <flint-speed-dial-action name="print" tooltip-title="Print">🖨️</flint-speed-dial-action>
                        <flint-speed-dial-action name="share" tooltip-title="Share">🔗</flint-speed-dial-action>
                        <flint-speed-dial-action name="save"  tooltip-title="Save">💾</flint-speed-dial-action>
                    </flint-speed-dial>
                </div>
            </div>
        </div>
...
export const KeyboardNavigation: Story = {
    render: () => html`
        <flint-stack direction="row" gap="24px" alignItems="flex-start" style="font-family:Inter,sans-serif;flex-wrap:wrap;">

            <!-- Cheat sheet -->
            <div style="
                flex:0 0 auto;padding:16px 20px;
                background:var(--flint-background, #fff);border:1px solid #e2e8f0;border-radius:8px;
                        ...
                        ${([
                            ...
                            <tr>
                                <td style="padding:4px 8px 4px 0;white-space:nowrap;">
                                    <kbd style="
                                        background:var(--flint-muted-background-light, #f1f5f9);border:1px solid #cbd5e1;
