...
import uiImageListItemStyles from './flint-image-list-item.css?inline';
...
export class FlintImageListItem extends FlintElement {
  ...
  private _applyHostStyles() {
    ...
    const parent = this.closest('flint-image-list');
    ...
    if (variant === 'masonry') {
      this.classList.add('masonry'); // wc/no-self-class: intentional host class
    } else {
      this.classList.remove('masonry'); // wc/no-self-class: intentional host class
    }
    ...
    this.style.setProperty('--flint-image-fit', this.fit);
  }
...
declare global {
  interface HTMLElementTagNameMap {
    'flint-image-list-item': FlintImageListItem;
  }
}
