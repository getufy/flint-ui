...
export const BasicUsage: Story = {
    ...
    <p style="font-size:14px;color:#555;margin-bottom:12px;">
      Pass an <code>items</code> array. By default, <code>ui-rich-tree-view</code>
      reads <code>id</code>, <code>label</code>, and <code>children</code> from each object.
    </p>
    <ui-rich-tree-view .items=${FILESYSTEM_ITEMS}></ui-rich-tree-view>
  `,
};
...
export const FocusableDisabledItems: Story = {
    ...
    render: (args) => {
        const toggle = (e: Event) => {
            ...
            const tree = document.getElementById('rich-focusable-tree') as UiRichTreeView | null;
...
export const TrackItemClicks: Story = {
    ...
    render: () => {
        ...
        return html`
      <ui-rich-tree-view .items=${FILESYSTEM_ITEMS} .onItemClick=${handleClick}></ui-rich-tree-view>
...
export const GetItemById: Story = {
    ...
    render: () => {
        const lookup = () => {
            const tree = document.getElementById('rich-get-item-tree') as UiRichTreeView | null;
...
export const GetItemDOMElement: Story = {
    ...
    render: () => {
        const highlight = () => {
            const tree = document.getElementById('rich-dom-tree') as UiRichTreeView | null;
            ...
            tree.shadowRoot?.querySelectorAll('ui-tree-item').forEach(item => {
            ...
            if (el) {
                ...
                output.textContent = `Found: <ui-tree-item item-id="${id}">`;
...
export const GetItemTree: Story = {
    ...
    render: () => {
        const showTree = () => {
            const tree = document.getElementById('rich-tree-api') as UiRichTreeView | null;
...
export const ImperativelyDisableItem: Story = {
    ...
    render: () => {
        const apply = () => {
            const tree = document.getElementById('rich-disable-tree') as UiRichTreeView | null;
...
export const ControlledExpansion: Story = {
    ...
    render: () => {
        ...
        const onExpandedItemsChange = (ids: string[]) => {
            expandedItems = ids;
            const tree = document.getElementById('rich-controlled-tree') as UiRichTreeView | null;
