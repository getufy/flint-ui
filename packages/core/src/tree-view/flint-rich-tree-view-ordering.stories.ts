...
export const LimitReordering: Story = {
  ...
  render: () => {
    const isItemReorderable = (itemId: string) => {
      ...
      const tree = document.getElementById('leaf-reorder-tree') as FlintRichTreeView | null;
      if (!tree) return true;
...
export const RestrictDropPositions: Story = {
  ...
  render: () => {
    const canMoveItemToNewPosition = (params: {
      ...
      const tree = document.getElementById('restrict-tree') as FlintRichTreeView | null;
      if (!tree) return true;
...
export const SyncTwoTrees: Story = {
  ...
  render: () => {
    const handlePositionChange = () => {
      const tree1 = document.getElementById('tree-sync-1') as FlintRichTreeView | null;
      const tree2 = document.getElementById('tree-sync-2') as FlintRichTreeView | null;
      if (tree1 && tree2) {
