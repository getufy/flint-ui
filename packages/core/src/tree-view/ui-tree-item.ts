 ...
 *
 * @slot - Place child `ui-tree-item` elements here to create a nested tree.
 *
 * @fires ui-tree-item-click  - Fired when the item is clicked (detail: { itemId })
 * @fires ui-tree-item-toggle - Fired when expanded state changes (detail: { itemId, expanded })
 */
...
export class UiTreeItem extends LitElement {
  ...
  /**
   * When `true`, forces the expand button to render even if no `ui-tree-item`
   * children are currently in the slot. Used by `ui-rich-tree-view` for lazy
  ...
  private _onSlotChange(e: Event) {
    ...
    const hasChildren = assigned.some(el => el.tagName === 'UI-TREE-ITEM');
  ...
  private _toggleExpand(e: MouseEvent) {
    ...
    this.dispatchEvent(new CustomEvent('ui-tree-item-toggle', {
  ...
  private _handleRowClick() {
    if (this.disabled) return;
    this.dispatchEvent(new CustomEvent('ui-tree-item-click', {
  ...
  private get _level(): number {
    ...
    while (parent) {
      if (parent.tagName === 'UI-TREE-ITEM') level++;
      else if (parent.tagName === 'UI-SIMPLE-TREE-VIEW') break;
