...
import uiScrollAreaStyles from './ui-scroll-area.css?inline';
import uiScrollBarStyles from './ui-scroll-bar.css?inline';
...
/**
 * Custom overlay scrollbar. Place inside `ui-scroll-area` with
 * `slot="scrollbar"` for an explicit horizontal or second-axis bar.
 *
 * The parent `ui-scroll-area` calls `setThumb()` and `setVisible()` to
 ...
 *
 * @cssprop --ui-scrollbar-size              - Bar track width/height (default: `8px`)
 * @cssprop --ui-scrollbar-thumb-color       - Thumb fill (default: `rgba(0,0,0,.35)`)
 * @cssprop --ui-scrollbar-thumb-hover-color - Thumb fill on hover/drag
 * @cssprop --ui-scrollbar-thumb-radius      - Thumb border radius (default: `9999px`)
 * @cssprop --ui-scrollbar-track-color       - Track background (default: `transparent`)
 */
@customElement('ui-scroll-bar')
export class UiScrollBar extends LitElement {
    ...
    private get _area(): UiScrollArea | null {
        return this.closest('ui-scroll-area') as UiScrollArea | null;
    }
 ...
 * @slot scrollbar - Optional explicit `ui-scroll-bar` elements (e.g. horizontal).
 *
 ...
 *
 * @cssprop --ui-scrollbar-size              - Bar width/height (default: `8px`)
 * @cssprop --ui-scrollbar-thumb-color       - Thumb fill (default: `rgba(0,0,0,.35)`)
 * @cssprop --ui-scrollbar-thumb-hover-color - Thumb hover/drag fill
 * @cssprop --ui-scrollbar-thumb-radius      - Thumb border radius (default: `9999px`)
 * @cssprop --ui-scrollbar-track-color       - Track background (default: `transparent`)
 */
@customElement('ui-scroll-area')
export class UiScrollArea extends LitElement {
    ...
    private _syncScrollBars() {
        ...
        // Sync any slotted ui-scroll-bar elements
        this.querySelectorAll('ui-scroll-bar').forEach(el => {
            if ((el as Element).closest('ui-scroll-area') !== this) return;
            const bar = el as UiScrollBar;
