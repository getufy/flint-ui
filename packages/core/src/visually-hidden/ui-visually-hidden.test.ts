...
import type { UiVisuallyHidden } from './ui-visually-hidden';

/* ── helpers ─────────────────────────────────────────────────────── */

async function make(slotContent = html`<span>Hidden text</span>`) {
    const el = await fixture<UiVisuallyHidden>(html`
        <ui-visually-hidden>${slotContent}</ui-visually-hidden>
    `);
...
/* ═══════════════════════════════════════════════════════════════════
   Rendering
═══════════════════════════════════════════════════════════════════ */
describe('ui-visually-hidden — rendering', () => {
    ...
    it('accepts plain text as slotted content', async () => {
        const el = await fixture<UiVisuallyHidden>(html`
            <ui-visually-hidden>Plain text content</ui-visually-hidden>
        `);
        await el.updateComplete;
...
/* ═══════════════════════════════════════════════════════════════════
   Accessibility
═══════════════════════════════════════════════════════════════════ */
describe('ui-visually-hidden — accessibility', () => {
...
/* ═══════════════════════════════════════════════════════════════════
   DOM structure
═══════════════════════════════════════════════════════════════════ */
describe('ui-visually-hidden — DOM structure', () => {
    ...
    it('tag name is ui-visually-hidden', async () => {
        const el = await make();
        expect(el.tagName.toLowerCase()).toBe('ui-visually-hidden');
    });
...
═══════════════════════════════════════════════════════════════════ */
describe('ui-visually-hidden — notFocusable prop', () => {
    ...
    it('setting not-focusable attribute reflects to notFocusable property', async () => {
        const el = await fixture<UiVisuallyHidden>(html`
            <ui-visually-hidden not-focusable><span>SR text</span></ui-visually-hidden>
        `);
        await el.updateComplete;
    ...
    it('setting notFocusable = false removes the attribute', async () => {
        const el = await fixture<UiVisuallyHidden>(html`
            <ui-visually-hidden not-focusable><span>text</span></ui-visually-hidden>
        `);
        await el.updateComplete;
...
═══════════════════════════════════════════════════════════════════ */
describe('ui-visually-hidden — nesting & multiple instances', () => {
    it('two independent instances do not share state', async () => {
        const container = await fixture<HTMLDivElement>(html`
            <div>
                <ui-visually-hidden id="vh1"><span>Label one</span></ui-visually-hidden>
                <ui-visually-hidden id="vh2" not-focusable><span>Label two</span></ui-visually-hidden>
            </div>
        `);
        const vh1 = container.querySelector('#vh1') as UiVisuallyHidden;
        const vh2 = container.querySelector('#vh2') as UiVisuallyHidden;
    ...
    it('multiple instances each render their own slot', async () => {
        const container = await fixture<HTMLDivElement>(html`
            <div>
                <ui-visually-hidden><span id="c1">First</span></ui-visually-hidden>
                <ui-visually-hidden><span id="c2">Second</span></ui-visually-hidden>
            </div>
        `);
    ...
    it('can be nested inside other elements', async () => {
        const label = await fixture<HTMLLabelElement>(html`
            <label>
                <ui-visually-hidden>Email address</ui-visually-hidden>
                <input type="email" placeholder="Email" />
            </label>
        `);
        const vh = label.querySelector('ui-visually-hidden')!;
    ...
    it('can be used inside a button', async () => {
        const btn = await fixture<HTMLButtonElement>(html`
            <button>
                ...
                <ui-visually-hidden not-focusable>Close dialog</ui-visually-hidden>
            </button>
        `);
        const vh = btn.querySelector('ui-visually-hidden')!;
        expect(vh).not.toBeNull();
...
/* ═══════════════════════════════════════════════════════════════════
   Typical use-cases
═══════════════════════════════════════════════════════════════════ */
describe('ui-visually-hidden — use cases', () => {
    ...
    it('icon-button label use-case: hidden span text matches', async () => {
        const el = await fixture<UiVisuallyHidden>(html`
            <ui-visually-hidden not-focusable><span>Delete item</span></ui-visually-hidden>
        `);
        await el.updateComplete;
    ...
    it('breadcrumb separator use-case: hidden text replaces aria-hidden separator', async () => {
        ...
        `);
        const vh = nav.querySelector('ui-visually-hidden')!;
    ...
    it('status indicator use-case: hidden status text is in DOM', async () => {
        ...
        `);
        const vh = container.querySelector('ui-visually-hidden')!;
        const text = Array.from(vh.childNodes)
            .filter(n => n.nodeType === Node.TEXT_NODE)
            .map(n => n.textContent?.trim())
            .join('');
