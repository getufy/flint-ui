...
export class FlintNavigationMenuItem extends FlintElement {
    ...
    private _openContent() {
        const trigger = this.querySelector('flint-navigation-menu-trigger') as FlintNavigationMenuTrigger | null;
        if (!trigger || trigger.disabled) return;
        const contentId = trigger.contentId;
        if (!contentId) return;
        const content = this.querySelector(`[id="${contentId}"]`) as FlintNavigationMenuContent | null;
        if (!content) return;
        // Fire the standard trigger event so flint-navigation-menu can coordinate one-at-a-time
    ...
    private _syncChildren = () => {
        const triggers = this.querySelectorAll('flint-navigation-menu-trigger') as NodeListOf<FlintNavigationMenuTrigger>;
        const links = this.querySelectorAll('flint-navigation-menu-link') as NodeListOf<HTMLElement & { disabled: boolean }>;
        triggers.forEach((el) => {
            if (el.closest('flint-navigation-menu-item') === this) {
