 ...
 *
 * Unlike `ui-simple-tree-view` (which uses a slot), `ui-rich-tree-view` renders
 * `ui-tree-item` elements internally from data.
 *
...
export class UiRichTreeView extends LitElement {
    ...
    connectedCallback() {
        ...
        this.shadowRoot!.addEventListener('ui-tree-item-click', this._handleItemClick as EventListener);
        this.shadowRoot!.addEventListener('ui-tree-item-toggle', this._handleUiToggle as EventListener);
    ...
    disconnectedCallback() {
        ...
        this.shadowRoot!.removeEventListener('ui-tree-item-click', this._handleItemClick as EventListener);
        this.shadowRoot!.removeEventListener('ui-tree-item-toggle', this._handleUiToggle as EventListener);
    ...
    protected updated(_changedProperties: PropertyValues) {
        ...
        // shadowed by a Lit @property — the fix lives in ui-tree-item.ts.
    ...
    private _getAllItems(): UiTreeItem[] {
        return Array.from(this.shadowRoot!.querySelectorAll('ui-tree-item')) as UiTreeItem[];
    }
     ...
     */
    getItemDOMElement(id: string): UiTreeItem | null {
    ...
    private _getTreeItemFromEvent(e: DragEvent): UiTreeItem | null {
        ...
        for (const target of path) {
            if (target instanceof HTMLElement && target.tagName === 'UI-TREE-ITEM') {
                return target as UiTreeItem;
            }
        }
        // Fallback: if the event was dispatched directly on the ui-tree-item host
        // (as done in tests), e.target IS the element we want.
        const t = e.target as HTMLElement | null;
        if (t && t.tagName === 'UI-TREE-ITEM') return t as unknown as UiTreeItem;
        // Also check immediate parent in case event landed on a child element.
        const p = t?.closest?.('ui-tree-item') as UiTreeItem | null;
        return p ?? null;
    }
...
declare global {
    interface HTMLElementTagNameMap {
        'ui-rich-tree-view': UiRichTreeView;
    }
}