...
describe('ui-drawer', () => {
    ...
    it('Escape when drawer is closed does not dispatch ui-drawer-close', async () => {
        const el = await fixture<UiDrawer>(html`<ui-drawer></ui-drawer>`);
        const closeSpy = vi.fn();
        el.addEventListener('ui-drawer-close', closeSpy);
        window.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape' }));
    ...
    it('non-Escape key when open does not dispatch ui-drawer-close', async () => {
        const el = await fixture<UiDrawer>(html`<ui-drawer open></ui-drawer>`);
        const closeSpy = vi.fn();
        el.addEventListener('ui-drawer-close', closeSpy);
    ...
    it('ArrowDown key when open does not dispatch ui-drawer-close', async () => {
        const el = await fixture<UiDrawer>(html`<ui-drawer open></ui-drawer>`);
        const closeSpy = vi.fn();
        el.addEventListener('ui-drawer-close', closeSpy);
    ...
    it('Escape on persistent variant does not dispatch ui-drawer-close', async () => {
        const el = await fixture<UiDrawer>(html`<ui-drawer open variant="persistent"></ui-drawer>`);
        const closeSpy = vi.fn();
        el.addEventListener('ui-drawer-close', closeSpy);
        window.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape' }));
        expect(closeSpy).not.toHaveBeenCalled();
    ...
    it('Escape on mini variant does not dispatch ui-drawer-close', async () => {
        const el = await fixture<UiDrawer>(html`<ui-drawer open variant="mini"></ui-drawer>`);
        const closeSpy = vi.fn();
        el.addEventListener('ui-drawer-close', closeSpy);
        window.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape' }));
        expect(closeSpy).not.toHaveBeenCalled();
    ...
    it('ui-drawer-close event has bubbles=true and composed=true', async () => {
        const el = await fixture<UiDrawer>(html`<ui-drawer open></ui-drawer>`);
        let capturedEvent: CustomEvent | null = null;
        el.addEventListener('ui-drawer-close', (e) => { capturedEvent = e as CustomEvent; });
        const backdrop = el.shadowRoot!.querySelector('.backdrop') as HTMLElement;
        backdrop.click();
    ...
    it('ui-drawer-close event bubbles to a parent element', async () => {
        const parent = await fixture<HTMLDivElement>(html`
            <div><ui-drawer open></ui-drawer></div>
        `);
        const el = parent.querySelector('ui-drawer') as UiDrawer;
        const closeSpy = vi.fn();
        parent.addEventListener('ui-drawer-close', closeSpy);
        const backdrop = el.shadowRoot!.querySelector('.backdrop') as HTMLElement;
        backdrop.click();
        expect(closeSpy).toHaveBeenCalled();
    });
    ...
    it('edge handle has edge-right class for right anchor', async () => {
        const el = await fixture<UiDrawer>(html`<ui-drawer edge anchor="right"></ui-drawer>`);
        const edge = el.shadowRoot!.querySelector('.edge');
        expect(edge?.classList.contains('edge-right')).toBe(true);
    });

    it('edge handle has edge-top class for top anchor', async () => {
        const el = await fixture<UiDrawer>(html`<ui-drawer edge anchor="top"></ui-drawer>`);
        const edge = el.shadowRoot!.querySelector('.edge');
        expect(edge?.classList.contains('edge-top')).toBe(true);
    });

    it('edge handle has edge-bottom class for bottom anchor', async () => {
        const el = await fixture<UiDrawer>(html`<ui-drawer edge anchor="bottom"></ui-drawer>`);
        const edge = el.shadowRoot!.querySelector('.edge');
    ...
    it('mini variant close does not restore focus to a previous element', async () => {
        const trigger = await fixture<HTMLButtonElement>(html`<button>Toggle</button>`);
        trigger.focus();
        const el = await fixture<UiDrawer>(html`<ui-drawer variant="mini" open></ui-drawer>`);
        el.open = false;
        await el.updateComplete;
    ...
    it('anchor reflects to attribute', async () => {
        const el = await fixture<UiDrawer>(html`<ui-drawer anchor="right"></ui-drawer>`);
