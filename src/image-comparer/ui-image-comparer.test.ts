...
import type { UiImageComparer } from './ui-image-comparer';
...
async function make({ position = 50, disabled = false }: MakeOpts = {}) {
    const el = await fixture<UiImageComparer>(html`
        <ui-image-comparer .position=${position} ?disabled=${disabled}>
...
function shadow(el: UiImageComparer) {
    return el.shadowRoot!;
}

function handle(el: UiImageComparer) {
    return shadow(el).querySelector('.handle') as HTMLElement;
}

function divider(el: UiImageComparer) {
    return shadow(el).querySelector('.divider') as HTMLElement;
}

function beforePanel(el: UiImageComparer) {
    return shadow(el).querySelector('.before') as HTMLElement;
}

/* ── rendering ───────────────────────────────────────────────── */

describe('ui-image-comparer', () => {
    describe('rendering', () => {
    ...
    describe('disabled', () => {
        ...
        it('does not fire events when disabled and key pressed', async () => {
            ...
            const spy = vi.fn();
            el.addEventListener('ui-image-comparer-change', spy);
    ...
    describe('events', () => {
        it('fires ui-image-comparer-change with detail.position', async () => {
            const el = await make({ position: 50 });
            const spy = vi.fn();
            el.addEventListener('ui-image-comparer-change', spy);
    ...
    describe('pointer drag', () => {
        function mockRect(el: UiImageComparer) {
        ...
        it('fires ui-image-comparer-change during drag', async () => {
            const el = await make({ position: 50 });
            mockRect(el);
            const spy = vi.fn();
            el.addEventListener('ui-image-comparer-change', spy);
